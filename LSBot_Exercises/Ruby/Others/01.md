<p align="right">
  <a id="spot-launch-btn" class="spot-editor-link" href="https://launchschool.com/the-spot?url=https://github.com/The-SPOT-Hub/SPOT-Wiki/blob/main/LSBot_Exercises/Ruby/Others/01.md" target="_blank">
...
- `LRUCache.new(capacity)` — Initialize the cache with a positive `capacity`.
...
Ruby's `Hash` preserves insertion order and offers O(1) `delete` and re-insert. Use it to track recency: whenever a key is accessed, delete and re-insert it so it lands at the "most recently used" end, and when over capacity, evict the first key.
...
class LRUCache
  ...
  def get(key)
    ...
    @store[key] = value # re-insert moves key to the "most recently used" end
    value
  end
  ...
  def put(key, value)
    ...
    @store.delete(@store.first[0]) if @store.size > @capacity
    nil # return void, matching the :rtype: Void signature
  end
end
```
